{ inputs, ... }:
{
  perSystem =
    ...
    let
      ...
      inherit (import ../../nix/lib/users.nix { inherit lib pkgs; })
        passwdContents
        shadowContents
        groupContents
        ;

      inherit (import ../../nix/lib/config.nix { inherit pkgs; }) nixConfContents sshdConfig;

      # Import packages
      inherit (import ../../nix/packages/environment.nix { inherit pkgs lib; })
        cudaEnv
        corePkgs
        devPkgs
        networkPkgs
      ...
      inherit
        (import ../../nix/packages/system.nix {
          inherit
            pkgs
            lib
            passwdContents
